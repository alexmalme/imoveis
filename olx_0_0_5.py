...
class Parametros:
    def __init__(
        ...
        self.preco_minimo = (
            str(preco_minimo) if str(preco_minimo) != None else ''
            )
        self.preco_maximo = (
            str(preco_maximo) if str(preco_maximo) != None else ''
            )
        self.ordem = ordem
        self.pagina = (
    ...
    def set_url_default(
        ...
        u_compacto = (
            url_compacto if url_compacto != None else self.get_url_compacto
            )
        z = zona if zona != None else self.get_zona
...
# %%
    def choose_preco_maximo(self):
        ...
        while (
            len(preco_maximo) > 1 
            and int(preco_maximo) < int(self.get_preco_minimo)
               ):
            preco_maximo = input(
...
class Urls(Parametros):
    ...
    def parse_detail(self, url):
        try:            
            ...
            TipoAnunciante = (
                'proprietario' 
                if fields['ad']['professionalAd'] == False 
                else 'profissional'
                )
            AnuncioPortal = 'OLX'
            VagasBool = (
