...
class Urls(Parametros):
    ...
    def __init__(
        ...
        """
        Igual ao __init__ da classe Parâmetros
        """
        self.state = state
        self.start_url = start_url
        self.city = city
...
# %%
    def parse(self, url_completo, url_compacto, dbpath, count=0):
        print('-' * 30)
        print(url_completo)
        print('-' * 30, '\n' * 2)
        driver = self.driver_()
        ...
        if next_page:
            ...
            print('-' * 50)
            driver.close()
            return self.parse(next_page, url_compacto, dbpath, count=0)
        else:
                ...
                if r is not None:
                    ...
                    if vals[-1] - vals[-2] <= 50:
                        ...
                        if sql:
                            ...
                            driver.close()
                            return self.parse(self.get_url_completo, self.get_url_compacto, dbpath)
                        else:
                            driver.close()
                            print(
                    ...
                    if count >= 3:
                        ...
                        return self.parse(url_completo, url_compacto, dbpath, count + 1)
...
# %%
    def process_item(self, item, dbpath):
...
#   u = Urls(parametros.url_default)
    dbpath = 'caminho para o banco de dados'
